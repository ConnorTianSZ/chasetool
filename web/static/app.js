...
document.addEventListener('alpine:init', () => {
  ...
  Alpine.data('inbox', (projectId) => ({
    ...
    _isHtml(str) {
      ...
      return s.startsWith('<!doctype') || s.startsWith('<html') || /<[a-z][\s\S]*>/i.test(s);
    },

    _emailBodyStyle() {
      return 'font-family:Arial,"Microsoft YaHei",sans-serif;font-size:13px;color:#111827;padding:12px;line-height:1.5;overflow-wrap:break-word;';
    },
...
<table style="border-collapse:collapse;margin-bottom:16px;"><tr>${tds}</tr></table>`;
    ...
    _buildBuyerTableHtml() {
      ...
      const thStyle = 'border:1px solid #d1d5db;background:#f3f4f6;padding:3px 6px;text-align:left;font-size:12px;line-height:1.2;';
      const tdStyle = 'border:1px solid #e5e7eb;padding:2px 6px;font-size:12px;line-height:1.2;';
...
<table style="border-collapse:collapse;width:100%;margin-bottom:16px;"><thead><tr>${hs}</tr></thead><tbody>${rows}</tbody></table>`;
    ...
    _buildOnePivotAHtml(pivotData, label) {
      ...
      const thStyle = 'border:1px solid #d1d5db;background:#f3f4f6;padding:2px 4px;text-align:center;font-size:11px;line-height:1.2;';
      const tdStyle = 'border:1px solid #e5e7eb;padding:2px 4px;text-align:center;font-size:12px;line-height:1.2;';
      ...
      const rows = pivotData.buyers.map(b => {
        ...
        return `<tr><td style="border:1px solid #e5e7eb;padding:2px 6px;font-weight:600;font-size:12px;line-height:1.2;">${this._he(b)}</td>${tds}<td style="${tdStyle}font-weight:700;background:#f9fafb;">${pivotData.row_totals[b]||0}</td></tr>`;
      }).join('');
...
<tbody>${rows}<tr><td style="border:1px solid #d1d5db;padding:2px 6px;font-weight:700;background:#f3f4f6;font-size:12px;line-height:1.2;">合计</td>${colTotals}<td style="${tdStyle}font-weight:700;background:#f3f4f6;">${grand}</td></tr></tbody></table>`;
    },
...
<tbody>${rows}<tr><td style="border:1px solid #d1d5db;padding:2px 6px;font-weight:700;background:#f3f4f6;font-size:12px;line-height:1.2;">合计</td>${colTotals}<td style="${tdStyle}font-weight:700;background:#f3f4f6;">${grand}</td></tr></tbody></table>`;
    ...
    _buildPivotBHtml() {
      if (!this.pivotB?.rows?.length) return '';
      const thStyle = 'border:1px solid #d1d5db;background:#f3f4f6;padding:3px 6px;font-size:12px;line-height:1.2;';
      const rows = this.pivotB.rows.map(r => {
        const mfrRows = r.manufacturers.map(m=>
          `<tr><td style="border:1px solid #e5e7eb;padding:2px 6px 2px 28px;font-size:12px;color:#374151;line-height:1.2;">└ ${this._he(m.name)}</td><td style="border:1px solid #e5e7eb;padding:2px 6px;text-align:center;font-size:12px;line-height:1.2;">${m.count}</td></tr>`
        ).join('');
        return `<tr style="background:#f9fafb;"><td style="border:1px solid #e5e7eb;padding:3px 6px;font-weight:700;font-size:12px;line-height:1.2;">${this._he(r.buyer)}</td><td style="border:1px solid #e5e7eb;padding:3px 6px;text-align:center;font-weight:700;color:#7c3aed;font-size:12px;line-height:1.2;">${r.total}</td></tr>${mfrRows}`;
      }).join('');
